...
<body>
  ...
  <script>
    ...
    function calcMACD(prices) {
      ...
      const emaF = calcEMA(prices, 12);
      const emaS = calcEMA(prices, 26);
    ...
    function renderTA(data) {
      ...
      if (!section || data.length < 30) { if (section) section.innerHTML = ''; return; }
      ...
      const bbPct = calcBBPct(prices);
      const mom5 = calcMom(prices, 5);
      const mom10 = calcMom(prices, 10);

      // MA crossover
      const ma7v = prices.slice(-7).reduce((a, b) => a + b, 0) / 7;
      const ma30v = prices.length >= 30 ? prices.slice(-30).reduce((a, b) => a + b, 0) / 30 : null;
      const maCross = ma30v ? (ma7v > ma30v ? 'bullish' : 'bearish') : null;

      // Support / Resistance (20-day)
      const srSlice = vndPrices.slice(-20);
      ...
      if (maCross) {
        maSig = maCross;
        maLbl = maCross === 'bullish' ? 'MA7 > MA30' : 'MA7 < MA30';
        signals.push(maSig);
      }
      ...
      if (mom5 !== null) {
        if (mom5 > 2) { momSig = 'bullish'; momLbl = 'Tăng mạnh'; }
        else if (mom5 < -2) { momSig = 'bearish'; momLbl = 'Giảm mạnh'; }
        else if (mom5 > 0.5) { momSig = 'bullish'; momLbl = 'Tăng nhẹ'; }
        else if (mom5 < -0.5) { momSig = 'bearish'; momLbl = 'Giảm nhẹ'; }
        signals.push(momSig);
      }
      ...
      section.innerHTML =
        '<div class="ta-container">' +
          '<div class="ta-header">' +
            ...
            '<span class="ta-subtitle">' + data.length + ' phiên | Short-term signals</span>' +
          '</div>' +
          '<div class="ta-grid">' +
            ...
            '<div class="ta-card ' + taSignalClass(macdSig) + '">' +
              '<div class="ta-card-title">MACD (12,26,9)</div>' +
            ...
            '<div class="ta-card ' + taSignalClass(bbSig) + '">' +
              '<div class="ta-card-title">BB %B</div>' +
              '<div class="ta-card-value">' + fN(bbPct) + '%</div>' +
              '<div class="ta-card-signal">' + bbLbl + '</div>' +
            '</div>' +
            '<div class="ta-card ' + taSignalClass(momSig) + '">' +
              '<div class="ta-card-title">Momentum 5D</div>' +
              '<div class="ta-card-value">' + (mom5 !== null ? (mom5 >= 0 ? '+' : '') + fN(mom5) + '%' : 'N/A') + '</div>' +
              '<div class="ta-card-signal">' + momLbl + '</div>' +
              '<div class="ta-card-detail">' + (mom10 !== null ? '10D: ' + (mom10 >= 0 ? '+' : '') + fN(mom10) + '%' : '') + '</div>' +
            '</div>' +
          ...
          '<div class="ta-levels">' +
            '<div class="ta-level-item">' +
              '<span class="ta-level-label">Hỗ trợ (20D)</span>' +
            ...
            '</div>' +
            '<div class="ta-level-item">' +
              '<span class="ta-level-label">Kháng cự (20D)</span>' +
